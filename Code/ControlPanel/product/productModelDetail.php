...
if(isset($_POST["btnSave"])){
    ...
    $uploadFileTarget = "/ControlPanel/assets/images/model/";
    ...
    $sql = "";
    if(empty($_GET["ref"])){
        $genID = GenerateNextID("product_model","ModelCode",5,"M");
        $sql = "insert into product_model (ModelCode,ModelName,ModelDetail,Image,Active,CreatedOn,CreatedBy,BrandCode) values(
                '$genID'
                ,'$txtSubject'
                ,'$txtDetail'
                ,'$fileUploadedPath'
        ...
        $sql = "update product_model set
                ModelName = '$txtSubject'
                ,ModelDetail = '$txtDetail'
                ,Active = $chkActive
                ,Image = '$fileUploadedPath'
                ,UpdatedOn = NOW()
                ,UpdatedBy = '".UserService::UserCode()."'
                ,CategoryCode = '$ddlBrand'
                where ModelCode = '".$_GET["ref"]."'
        ";
    }
    
    ExecuteSQLTransaction($sql,"productModel.php");
}

if(!empty($_GET["ref"]))
{
    $sql = "select * from product_model where ModelCode = '".$_GET["ref"]."'";
    $data = SelectRow($sql);
}
?>
...
<div class="mat-box grey-bar">
    ...
    /
    <a href="productModel.php" class="link-history-btn">รายการรุ่น<?php echo $_COG_ITEM_NAME ?></a>
    /
    <span class="link-history-btn">จัดการรุ่น<?php echo $_COG_ITEM_NAME ?></span>
            ...
            <div class="col-md-12">
                ...
                <div class="row">
                    ...
                    <hr style="margin-top: 5px;" />
                        ...
                        <div class="row">
                            <div class="col-sm-6">
                                <label>ชื่อรุ่น<?php echo $_COG_ITEM_NAME ?></label>
                                <input type="text" name="txtSubject" id="txtSubject" value="<?php echo $data["ModelName"] ?>" class="form-control input-sm require" />
                            </div>
                            <div class="col-sm-6">
                                <label>ยี่ห้อ</label>
                                <select name="ddlBrand" class="form-control input-sm require">
                                    <?php echo GetDropDownListOptionWithDefaultSelectedAndCondition("product_brand","BrandCode","BrandName","กรุณาเลือก",$data["BrandCode"],"Active = 1") ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <textarea name="txtDetail" id="txtDetail" class="form-control input-sm"><?php echo $data["ModelDetail"] ?></textarea>
                            </div>
                        </div>
                ...
                <div>
                    ...
                    <a href="productModel.php" class="btn btn-danger">
