...
$prdCode = isset($_GET["ref"]) ? trim($_GET["ref"]) : '';
if (!empty($prdCode) && !preg_match('/^[a-zA-Z0-9_\-]+$/', $prdCode)) {
...
if (isset($_POST["btnSubmit"])) {
    ...
    $txtSubject           = esc($conn, $_POST["txtSubject"]           ?? '');
    $txtShortDescription  = esc($conn, $_POST["txtShortDescription"]  ?? '');
    $txtShortDescription2 = esc($conn, $_POST["txtShortDescription2"] ?? '');
    $txtDetail            = esc($conn, GeneratePageFile($_POST["txtDetail"]  ?? ''));
    $txtDetail2           = esc($conn, GeneratePageFile($_POST["txtDetail2"] ?? ''));
    $txtDetail3           = esc($conn, GeneratePageFile($_POST["txtDetail3"] ?? ''));
    $txtDetail4           = esc($conn, GeneratePageFile($_POST["txtDetail4"] ?? ''));
    $txtDetail5           = esc($conn, GeneratePageFile($_POST["txtDetail5"] ?? ''));
    $txtDetail6           = esc($conn, GeneratePageFile($_POST["txtDetail6"] ?? ''));
    $chkActive            = isset($_POST["chkActive"]) ? 1 : 0;

    $txtByName       = esc($conn, $_POST["txtByName"]      ?? '');
    $txtProductSize  = esc($conn, $_POST["txtProductSize"] ?? '');
    $txtAddress      = esc($conn, $_POST["txtAddress"]     ?? '');
    $txtLazada       = esc($conn, $_POST["txtLazada"]      ?? '');
    $txtShopee       = esc($conn, $_POST["txtShopee"]      ?? '');
    $txtLine         = esc($conn, $_POST["txtLine"]        ?? '');

    $txtAmount    = doubleval($_POST["txtAmount"]    ?? 0);
    $txtRefCode   = esc($conn, $_POST["txtRefCode"] ?? '');
    $txtBasePrice = doubleval(str_replace(",", "", $_POST["txtBasePrice"] ?? '0'));
    $txtOldPrice  = doubleval(str_replace(",", "", $_POST["txtOldPrice"]  ?? '0'));
    $txtPrice     = doubleval(str_replace(",", "", $_POST["txtPrice"]     ?? '0'));
    $txtPriceDesc = esc($conn, $_POST["txtPriceDesc"] ?? '');
    $txtWeight    = doubleval(str_replace(",", "", $_POST["txtWeight"] ?? '0'));
    ...
    $txtRank         = intval($_POST["txtRank"]         ?? 0);
    $ddlCategory     = esc($conn, $_POST["ddlCategory"]     ?? '');
    $ddlBrand        = esc($conn, $_POST["ddlBrand"]        ?? '');
    $ddlModel        = esc($conn, $_POST["ddlModel"]        ?? '');
    $ddlSubCategory  = esc($conn, $_POST["ddlSubCategory"]  ?? '');
    $ddlSubCategory3 = esc($conn, $_POST["ddlSubCategory3"] ?? '');
    $ddlSubCategory4 = esc($conn, $_POST["ddlSubCategory4"] ?? '');
    $txtLatLng       = esc($conn, $_POST["txtLatLng"]       ?? '');
    $ddlStatus       = esc($conn, $_POST["ddlStatus"]       ?? '');

    $hddFilePathVedio = esc($conn, $_POST["hddFilePathVedio"] ?? '');

    // ข้อมูลการใช้สอย
    $txtLandArea     = doubleval($_POST["txtLandArea"]     ?? 0);
    $txtUsableArea   = doubleval($_POST["txtUsableArea"]   ?? 0);
    $txtBedRoom      = doubleval($_POST["txtBedRoom"]      ?? 0);
    $txtToilet       = doubleval($_POST["txtToilet"]       ?? 0);
    $txtParkingSpace = doubleval($_POST["txtParkingSpace"] ?? 0);
    $txtFloorNumber  = doubleval($_POST["txtFloorNumber"]  ?? 0);
    $txtLivingRoom   = doubleval($_POST["txtLivingRoom"]   ?? 0);
    $txtKitchenRoom  = doubleval($_POST["txtKitchenRoom"]  ?? 0);

    $txtStartDateType = !empty($_POST["txtStartDate"])
        ? new DateTime(ConvertDateTimeDisplayToDateTimeDB($_POST["txtStartDate"]) . " 00:00:00.000000")
        : new DateTime();
    $txtStartDate = $txtStartDateType->format('Y-m-d H:i:s');

    $txtEndDateType = !empty($_POST["txtEndDate"])
        ? new DateTime(ConvertDateTimeDisplayToDateTimeDB($_POST["txtEndDate"]) . " 00:00:00.000000")
        : new DateTime();
    $txtEndDate = $txtEndDateType->format('Y-m-d H:i:s');

    $txtCustomer   = esc($conn, $_POST["txtCustomer"]   ?? '');
    $txtTaxInfo    = esc($conn, $_POST["txtTaxInfo"]    ?? '');
    $txtVendor     = esc($conn, $_POST["txtVendor"]     ?? '');
    $txtCollection = esc($conn, $_POST["txtCollection"] ?? '');
    $txtBarcode    = esc($conn, $_POST["txtBarcode"]    ?? '');
    $txtColorCode  = esc($conn, $_POST["txtColorCode"]  ?? '');
    $txtColorCode2 = esc($conn, $_POST["txtColorCode2"] ?? '');

    // Array fields
    $allColor        = esc($conn, implode(",", array_filter((array)($_POST["chkTag"]          ?? []))));
    $allSize         = esc($conn, implode(",", array_filter((array)($_POST["chkSize"]         ?? []))));
    $allMat          = esc($conn, implode(",", array_filter((array)($_POST["chkMaterial"]     ?? []))));
    $allKeyWord      = esc($conn, implode(",", array_filter((array)($_POST["chkTagKeyWord"]   ?? []))));
    $allFacility     = esc($conn, implode(",", array_filter((array)($_POST["chkFacility"]     ?? []))));
    $allDesign       = esc($conn, implode(",", array_filter((array)($_POST["chkDesign"]       ?? []))));
    $allProjectModel = esc($conn, implode(",", array_filter((array)($_POST["chkProjectModel"] ?? []))));

    // ============================================================
    // SECURITY FIX: File Upload - ใช้ safeUploadFile()
    // ============================================================
    //FILE 1
    ...
    if (!empty($prdCode)) {
        ...
        $sqlInsert = "INSERT INTO product (
                        ProductCode,ProductName,ShortDescription,ShortDescription2,
                        ProductDetail,ProductDetail2,ProductDetail3,ProductDetail4,ProductDetail5,ProductDetail6,
                        Image,Image2,Image3,Image4,Video,Video2,
                        FileDownload,FileDownload2,FileDownload3,
                        FileDownloadName,FileDownloadName2,FileDownloadName3,
                        Active,CreatedOn,CreatedBy,
                        BasePrice,OldPrice,Price,
                        Hot,Promotion,New,Recommend,Rank,
                        CategoryCode,Size,Material,ProductRefCode,
                        Color,Tag,Facility,ProjectModel,DesignModel,
                        BrandCode,ModelCode,SubCategoryCode,SubCategoryCode3,SubCategoryCode4,
                        MapLocation,Type,
                        LandArea,UsableArea,BedRoom,Toilet,ParkingSpace,FloorNumber,LivingRoom,KitchenRoom,
                        ByName,ProductSize,Amount,Address,PriceDesc,
                        StartDate,EndDate,Customer,Weight,LandingPage,
                        TaxInfo,Vendor,Collection,Barcode,ColorCode,ColorCode2,SEQ
                    ) VALUES (
                        '$prdCodeEsc','$txtSubject','$txtShortDescription','$txtShortDescription2',
                        '$txtDetail','$txtDetail2','$txtDetail3','$txtDetail4','$txtDetail5','$txtDetail6',
                        '$fileUploadedPath','$fileUploadedPath2','$fileUploadedPath3','$fileUploadedPath4',
                        '$hddFilePathVedio','$fileUploadedVideoPath2',
                        '$fileDownloadUploadPath','$fileDownloadUploadPath2','$fileDownloadUploadPath3',
                        '$fileDownloadName','$fileDownloadName2','$fileDownloadName3',
                        '$chkActive',NOW(),'" . UserService::UserCode() . "',
                        '$txtBasePrice','$txtOldPrice','$txtPrice',
                        '$choHot','$choPromotion','$choNew','$choRecommend','$txtRank',
                        '$ddlCategory','$allSize','$allMat','$txtRefCode',
                        '$allColor','$allKeyWord','$allFacility','$allProjectModel','$allDesign',
                        '$ddlBrand','$ddlModel','$ddlSubCategory','$ddlSubCategory3','$ddlSubCategory4',
                        '$txtLatLng','$ddlStatus',
                        $txtLandArea,$txtUsableArea,$txtBedRoom,$txtToilet,
                        $txtParkingSpace,$txtFloorNumber,$txtLivingRoom,$txtKitchenRoom,
                        '$txtByName','$txtProductSize',$txtAmount,'$txtAddress','$txtPriceDesc',
                        '$txtStartDate','$txtEndDate','$txtCustomer','$txtWeight',1,
                        '$txtTaxInfo','$txtVendor','$txtCollection','$txtBarcode',
                        '$txtColorCode','$txtColorCode2',$SEQ
                    )";

        ExecuteSQLTransaction($sqlInsert, "product.php");
    }

    $sqlDeleteColor = "DELETE FROM product_properties_mapping WHERE ProductCode = '$prdCodeEsc'";
    ExecuteSQL($sqlDeleteColor);

    if ($__COG_PROPERTIES_OPTION) {
        $chkPropDetail = (array)($_POST["chkPropDetail"] ?? []);
        foreach ($chkPropDetail as $tg) {
            if (!empty($tg)) {
                $tgEsc = esc($conn, $tg);
                $sqlInsertProp = "INSERT INTO product_properties_mapping (PropCode,ProductCode,Detail)
                                  VALUES('$tgEsc','$prdCodeEsc','1')";
                ExecuteSQL($sqlInsertProp);
            }
        }
    } else {
        $txtPropCode   = (array)($_POST["txtPropCode"]   ?? []);
        $txtPropDetail = (array)($_POST["txtPropDetail"] ?? []);
        for ($i = 0; $i < count($txtPropCode); $i++) {
            $propDetail = trim($txtPropDetail[$i] ?? '');
            if (empty($propDetail)) continue;
            $propCodeEsc   = esc($conn, $txtPropCode[$i]);
            $propDetailEsc = esc($conn, $propDetail);
            $sqlInsertProp = "INSERT INTO product_properties_mapping (PropCode,ProductCode,Detail)
                              VALUES('$propCodeEsc','$prdCodeEsc','$propDetailEsc')";
            ExecuteSQL($sqlInsertProp);
        }
    }
...
if (!empty($prdCode)) {
    ...
    $prdCodeEsc = mysqli_real_escape_string($connSel, $prdCode);
    $sqlPrd     = "SELECT * FROM product WHERE ProductCode = '$prdCodeEsc'";
                    ...
                    <div class="col-md-9">
                        ...
                        <div class="row">
                            <div class="col-sm-3 hide">
                                ...
                                <input type="text" placeholder="รหัส<?php echo $_COG_ITEM_NAME ?>..." name="txtRefCode" id="txtRefCode" value="<?php echo h($data["ProductRefCode"]) ?>" class="form-control input-sm" />
                            </div>
                            <div class="col-sm-6">
                                <label>ชื่อ<?php echo $_COG_ITEM_NAME ?></label>
                                <input type="text" placeholder="ชื่อ<?php echo $_COG_ITEM_NAME ?>..." name="txtSubject" id="txtSubject" value="<?php echo h($data["ProductName"]) ?>" class="form-control input-sm require" />
                            </div>
                            <div class="col-sm-6">
                            ...
                            <script>
                                ...
                                <?php if (!empty($prdCode)) { ?>
                                    ...
                                    loadSubCategory('<?php echo js($data["CategoryCode"]) ?>', '<?php echo js($data["SubCategoryCode"]) ?>');
                        ...
                        <div class="row hide">
                            ...
                            <script>
                                ...
                                <?php if (!empty($data["SubCategoryCode"])) { ?>
                                    loadSubCategory3('<?php echo js($data["SubCategoryCode"]) ?>', '<?php echo js($data["SubCategoryCode3"]) ?>');
                                <?php } ?>
                                <?php if (!empty($data["SubCategoryCode3"])) { ?>
                                    loadSubCategory4('<?php echo js($data["SubCategoryCode3"]) ?>', '<?php echo js($data["SubCategoryCode4"]) ?>');
                                <?php } ?>
                            </script>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <textarea name="txtShortDescription" id="txtShortDescription" class="form-control input-sm" rows="4"><?php echo h($data["ShortDescription"]) ?></textarea>
                            </div>
                        </div>
                        ...
                        <div class="row hide">
                            ...
                            <script>
                                ...
                                <?php if (!empty($prdCode)) { ?>
                                    ...
                                    loadModel('<?php echo js($data["BrandCode"]) ?>', '<?php echo js($data["ModelCode"]) ?>');
                        ...
                        <div class="row hide">
                            <!-- <div class="col-sm-6">
                                ...
                                <input type="text" placeholder="บริษัทก่อสร้าง..." name="txtByName" id="txtByName" value="<?php echo h($data["ByName"]) ?>" class="form-control input-sm require" />
                            </div> -->
                            <div class="col-sm-3 hide">
                                <label>ขนาดพื้นที่โครงการ</label>
                                <input type="text" placeholder="ขนาด..." name="txtProductSize" id="txtProductSize" value="<?php echo h($data["ProductSize"]) ?>" class="form-control input-sm require" />
                            </div>
                            <div class="col-sm-3">
                                <!-- <label>จำนวน (ยูนิต)</label> -->
                                <label>จำนวนสินค้า (คงเหลือ)</label>
                                <input type="number" placeholder="จำนวนคงเหลือ..." name="txtAmount" id="txtAmount" value="<?php echo h($data["Amount"]) ?>" class="form-control input-sm text-right require" />
                            </div>
                            <div class="col-sm-8">
                                <label>&nbsp;</label>
                                <div style="color:red">
                        ...
                        <div class="row hide">
                            <div class="col-sm-12">
                                ...
                                <textarea name="txtAddress" id="txtAddress" class="form-control input-sm require" rows="4"><?php echo h($data["Address"]) ?></textarea>
                            </div>
                        </div>
                                ...
                                <div class="alert alert-warning">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-6">
                                            ...
                                            <input type="number" step="1" onchange="weightCal(this);" onkeyup="weightChange(this);" placeholder="น้ำหนัก (กรัม)..." name="txtWeight" id="txtWeight" value="<?php echo h($data["Weight"]) ?>" class="form-control input-sm require" />
                                        </div>
                                        <div class="col-sm-6">
                        ...
                        <div class="alert alert-info hide">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <input type="number" placeholder="" name="txtUsableArea" id="txtUsableArea" value="<?php echo h($data["UsableArea"]) ?>" class="form-control input-sm text-right require" />
                                </div>
                                <div class="col-sm-4">
                                    <label>ขนาดที่ดิน (ตารางวา)</label>
                                    <input type="number" placeholder="" name="txtLandArea" id="txtLandArea" value="<?php echo h($data["LandArea"]) ?>" class="form-control input-sm text-right require" />
                                </div>
                                <div class="col-sm-4 hide">
                                    <label>จำนวนชั้น</label>
                                    <input type="number" placeholder="" name="txtFloorNumber" id="txtFloorNumber" value="<?php echo h($data["FloorNumber"]) ?>" class="form-control input-sm text-right require" />
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-4">
                                    <label>ห้องนอน</label>
                                    <input type="number" placeholder="" name="txtBedRoom" id="txtBedRoom" value="<?php echo h($data["BedRoom"]) ?>" class="form-control input-sm text-right require" />
                                </div>
                                <div class="col-sm-4">
                                    <label>ห้องน้ำ</label>
                                    <input type="number" placeholder="" name="txtToilet" id="txtToilet" value="<?php echo h($data["Toilet"]) ?>" class="form-control input-sm text-right require" />
                                </div>
                                <div class="col-sm-4">
                                    <label>ห้องรับแขก</label>
                                    <input type="number" placeholder="" name="txtLivingRoom" id="txtLivingRoom" value="<?php echo h($data["LivingRoom"]) ?>" class="form-control input-sm text-right require" />
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-4">
                                    <label>ห้องครัว</label>
                                    <input type="number" placeholder="" name="txtKitchenRoom" id="txtKitchenRoom" value="<?php echo h($data["KitchenRoom"]) ?>" class="form-control input-sm text-right require" />
                                </div>
                                <div class="col-sm-4">
                                    <label>ที่จอดรถ</label>
                                    <input type="number" placeholder="" name="txtParkingSpace" id="txtParkingSpace" value="<?php echo h($data["ParkingSpace"]) ?>" class="form-control input-sm text-right require" />
                                </div>
                            </div>
                        ...
                        <div class="alert alert-warning hide">
                            ...
                            <div class="row ">
                                <div class="col-sm-6">
                                    ...
                                    <input type="text" placeholder="ชื่อลูกค้า..." name="txtCustomer" id="txtCustomer" value="<?php echo h($data["Customer"]) ?>" class="form-control input-sm require" />
                                </div>
                                <div class="col-sm-6">
                                    <label>บริษัทที่ปรึกษาโครงการ</label>
                                    <input type="text" placeholder="บริษัทที่ปรึกษาโครงการ..." name="txtByName" id="txtByName" value="<?php echo h($data["ByName"]) ?>" class="form-control input-sm require" />
                                </div>
                            </div>
                            <div class="row ">
                                <div class="col-sm-4">
                                    <label>มูลค่าโครงการ</label>
                                    <input type="text" placeholder="มูลค่าโครงการ..." name="txtPriceDesc" id="txtPriceDesc" value="<?php echo h($data["PriceDesc"]) ?>" class="form-control input-sm require" />
                                </div>
                                <div class="col-sm-4">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <input type="text" placeholder="Tax Info..." name="txtTaxInfo" id="txtTaxInfo" value="<?php echo h($data["TaxInfo"]) ?>" class="form-control input-sm" />
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-4">
                                    <label>Vendor</label>
                                    <input type="text" placeholder="Vendor..." name="txtVendor" id="txtVendor" value="<?php echo h($data["Vendor"]) ?>" class="form-control input-sm" />
                                </div>
                                <div class="col-sm-4">
                                    <label>Collection</label>
                                    <input type="text" placeholder="Collection..." name="txtCollection" id="txtCollection" value="<?php echo h($data["Collection"]) ?>" class="form-control input-sm" />
                                </div>
                                <div class="col-sm-4">
                                    <label>Barcode</label>
                                    <input type="text" placeholder="Barcode..." name="txtBarcode" id="txtBarcode" value="<?php echo h($data["Barcode"]) ?>" class="form-control input-sm" />
                                </div>
                            </div>
                            <br>
                            <div class="hide">
                        ...
                        <div class="row hide">
                            <div class="col-sm-4">
                                <label>Lazada</label>
                                <input type="text" name="txtLazada" id="txtLazada" value="<?php echo h($data["Lazada"]) ?>" class="form-control input-sm" />
                            </div>
                            <div class="col-sm-4">
                                <label>Shopee</label>
                                <input type="text" name="txtShopee" id="txtShopee" value="<?php echo h($data["Shopee"]) ?>" class="form-control input-sm" />
                            </div>
                            <div class="col-sm-4">
                                <label>Line</label>
                                <input type="text" name="txtLine" id="txtLine" value="<?php echo h($data["Line"]) ?>" class="form-control input-sm" />
                            </div>
                        </div>
                        ...
                        <div class="row hide">
                            <div class="col-md-12">
                                ...
                                <div class="slide-banner" style="height: 400px;">
                                    ...
                                    <input type="hidden" name="hddBackUpImage4" value="<?php echo h($data["Image4"]) ?>" />
                        ...
                        <div class="row hide">
                            <div class="col-sm-12">
                                ...
                                <textarea name="txtShortDescription2" id="txtShortDescription2" class="form-control input-sm require" rows="4"><?php echo h($data["ShortDescription2"]) ?></textarea>
                            </div>
                        </div>
                        ...
                        <div class="row hide">
                            <div class="col-sm-6">
                                ...
                                <input type="color" name="txtColorCode" id="txtColorCode" value="<?php echo h($data["ColorCode"]) ?>" class="form-control input-sm require" />
                            </div>
                            <div class="col-sm-6">
                                <label>สีพื้นหลังรายละเอียด Highlight</label>
                                <input type="color" name="txtColorCode2" id="txtColorCode2" value="<?php echo h($data["ColorCode2"]) ?>" class="form-control input-sm require" />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div>
                                ...
                                <div id="imge-preview" class="image-box hand" onclick="$(this).next().click();"
                                    style="margin:0 auto; width: 200px; height: 230px;margin-bottom:15px; background-image: url(<?php echo h($data["Image"]) ?>), url('../assets/images/default/570x679.png')">
                                </div>
                                <input class="hide" type="file" onchange="$(this).previewImage($('#imge-preview'));"
                                    name="fileUpload" id="fileUpload" accept="image/*" />
                                <input type="hidden" name="hddBackUpImage" value="<?php echo h($data["Image"]) ?>" />
                                <div class="text-center">
                                    <small>คลิกด้านบนรูปเพื่อเลือกรูปภาพ</small>
                                </div>
                            </div>
                            ...
                            <div class="hide">
                                ...
                                <div id="imge-preview-2" class="image-box hand" onclick="$(this).next().click();"
                                    style="margin:0 auto; width: 200px; height: 230px;margin-bottom:15px; background-image: url(<?php echo h($data["Image2"]) ?>), url('https://ipsumimage.appspot.com/800x800,eee')">
                                </div>
                                <input class="hide" type="file" onchange="$(this).previewImage($('#imge-preview-2'));"
                                    name="fileUpload2" id="fileUpload2" accept="image/*" />
                                <input type="hidden" name="hddBackUpImage2" value="<?php echo h($data["Image2"]) ?>" />
                                <div class="text-center">
                                    <small>คลิกด้านบนรูปเพื่อเลือกรูปภาพ</small>
                                </div>
                            </div>
                            ...
                            <div class="hide">
                                ...
                                <div id="imge-preview3" class="image-box hand" onclick="$(this).next().click();"
                                    style="margin:0 auto; width: 200px; height: 230px;margin-bottom:15px; background-image: url(<?php echo h($data["Image3"]) ?>), url('https://ipsumimage.appspot.com/558x369,eee')">
                                </div>

                                <input class="hide" type="file" onchange="$(this).previewImage($('#imge-preview3'));"
                                    name="fileUpload3" id="fileUpload3" accept="image/*" />

                                <input type="hidden" name="hddBackUpImage3" value="<?php echo h($data["Image3"]) ?>" />
                                            ...
                                            <div class="row panel-video">
                                                ...
                                                <div class="panel-video-display col-xs-12 <?php echo !empty($data["Video"]) ? "" : "hide" ?>">
                                                    <div class="slide-banner image-box">
                                                        <iframe class="iframe-video-diaplay" style="width:100%;height:100%;background:#000" src="<?php echo h($data["Video"]) ?>"></iframe>
                                                        ...
                                                        <input type="hidden" name="hddFilePathVedio" value="<?php echo h($data["Video"]) ?>" />
                                                    </div>
                                                </div>
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="row">
                                                                ...
                                                                <div class="col-sm-6">
                                                                    ...
                                                                    <div class="input-group">
                                                                        <input type="text" placeholder="URL Youtube" class="form-control input-sm require" name="txtImagePathVideo" value="<?php echo h($data["Video"]) ?>" />
                            ...
                            <div class="hide">
                                ...
                                <div class="row panel-video">
                                    ...
                                    <div class="panel-video-display col-xs-12 <?php echo !empty($data["Video2"]) ? "" : "hide" ?>">
                                        <div class="slide-banner image-box">
                                            ...
                                            <video class="slide-banner video-local-server" controls="" muted="" style="object-fit:cover;border-radius: 5px;">
                                                <source src="<?php echo h($data["Video2"]) ?>">
                                            </video>
                                    ...
                                    <input type="hidden" name="hddBackUpVideo" value="<?php echo h($data["Video2"]) ?>" />
                                        ...
                                        <?php if (!empty($data["FileDownload"])) { ?>
                                            <div style="margin-top:5px;">
                                                <b class="text-success">เคยอัพโหลดไฟล์แล้ว
                                                    <a target="_blank" href="<?php echo h($data["FileDownload"]); ?>">
                                                        <i class="fa fa-download"></i>
                                                        ดาวน์โหลด</a>
                                                </b>
                                            </div>
                                        <?php } ?>
                                        <input type="hidden" name="hddBackUpFileDownload" value="<?php echo h($data["FileDownload"]) ?>" />
                                        <input type="hidden" name="hddBackUpFileDownloadName" value="<?php echo h($data["FileDownloadName"]) ?>" />
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        ...
                                        <?php if (!empty($data["FileDownload2"])) { ?>
                                            <div style="margin-top:5px;">
                                                <b class="text-success">เคยอัพโหลดไฟล์แล้ว
                                                    <a target="_blank" href="<?php echo h($data["FileDownload2"]); ?>">
                                                        <i class="fa fa-download"></i>
                                                        ดาวน์โหลด</a>
                                                </b>
                                            </div>
                                        <?php } ?>
                                        <input type="hidden" name="hddBackUpFileDownload2" value="<?php echo h($data["FileDownload2"]) ?>" />
                                        <input type="hidden" name="hddBackUpFileDownloadName2" value="<?php echo h($data["FileDownloadName2"]) ?>" />
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        ...
                                        <?php if (!empty($data["FileDownload3"])) { ?>
                                            <div style="margin-top:5px;">
                                                <b class="text-success">เคยอัพโหลดไฟล์แล้ว
                                                    <a target="_blank" href="<?php echo h($data["FileDownload3"]); ?>">
                                                        <i class="fa fa-download"></i>
                                                        ดาวน์โหลด</a>
                                                </b>
                                            </div>
                                        <?php } ?>
                                        <input type="hidden" name="hddBackUpFileDownload3" value="<?php echo h($data["FileDownload3"]) ?>" />
                                        <input type="hidden" name="hddBackUpFileDownloadName3" value="<?php echo h($data["FileDownloadName3"]) ?>" />
                                    </div>
                                </div>
                            </div>
                            ...
                            ?>
                                <div class="product-location-container">
                                    <div style="margin-top: 5px;" class="">
                                        ...
                                        <hr style="margin-top: 5px; margin-bottom: 5px;" />
                                        <input readonly type="text" class="lat-lng form-control require input-sm" style="display:inline-block;float:left;margin-right:5px;width:200px;" value="<?php echo h($data["MapLocation"]) ?>" />
                                        <button type="button" class="btn btn-primary btn-sm" onclick="openSuperGridGoogleMapMap(this);">
                                            <i class="fa fa-map-marker text-danger"></i>
                                            ค้นหาตำแหน่งบนแผนที่
                                        </button>
                                        <input type="text" class="lat-lng hide" name="txtLatLng" value="<?php echo h($data["MapLocation"]) ?>" />
                                    </div>
                                </div>
                            <?php } ?>
