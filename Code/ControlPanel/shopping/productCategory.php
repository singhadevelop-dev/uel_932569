<?php $_COG_ITEM_CODE = 'SHOPPING'; ?>
<?php include  "../header.php"; ?>
<?php include  "_config.php"; ?>

<?php 

if(isset($_POST["btnDeleteRow"])){
    $sql = "delete from product_category where CategoryCode = '".$_POST["btnDeleteRow"]."'";
    ExecuteSQL($sql);
    
    $sql = "update product set CategoryCode = '' where CategoryCode = '".$_POST["btnDeleteRow"]."'";
    ExecuteSQL($sql);
}

?>



<div class="mat-box grey-bar">

    <a href="product.php" class="link-history-btn">หน้าหลักข้อมูล<?php echo $_COG_ITEM_NAME ?></a>
    /
    <span class="link-history-btn">รายการหมวดหมู่<?php echo $_COG_ITEM_NAME ?></span>

</div>


<div class="mat-box" style="border-radius: 0 0 3px 3px">
    <div class="row">
       <div class="col-md-2">
            <?php 
            $_LEFT_MENU_ACTIVE = "CATEGORY";
            include "leftMenu.php"; 
            ?>
        </div>
        <div class="col-md-10">
            <div>
                <a href="productCategoryDetail.php" class="pull-right">
                    <i class="fa fa-plus"></i>
                    เพิ่มรายการหมวดหมู่<?php echo $_COG_ITEM_NAME ?>
                </a>
                <span><b>รายการหมวดหมู่<?php echo $_COG_ITEM_NAME ?></b></span>
                <hr style="margin-top: 5px;" />
            </div>

            

            <table 
                data-sortable-table="product_category"
                data-sortable-column-seq="SEQ"
                data-sortable-column-key="CategoryCode"
                class="jquery-datatable sortable-table table table-striped table-hover">
                <thead>
                    <tr>
                        <th style="width:50px;">รหัส</th>
                        <th>ชื่อหมวดหมู่</th>
                        <!-- <th>กลุ่มหมวดหมู่</th> -->
                        <th style="width:50px;" class="text-center">ใช้งาน</th>
                        <th style="width:50px;" class="text-center">จัดการ</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr>
                        <th>รหัส</th>
                        <th>ชื่อหมวดหมู่</th>
                        <!-- <th>กลุ่มหมวดหมู่</th> -->
                        <th class="text-center">ใช้งาน</th>
                        <th class="text-center">จัดการ</th>
                    </tr>
                </tfoot>
                <tbody>
                    <?php
                    
                    $sql = "select * from product_category where CategoryGroup = 'SHOPPING' order by SEQ";
                    $datas = SelectRowsArray($sql);
                    foreach ($datas as $data) {
                    ?>
                    <tr data-sortable-row-key="<?php echo $data["CategoryCode"] ?>">
                        <td><?php echo $data["CategoryCode"] ?></td>
                        <td><?php echo $data["CategoryName"] ?></td>
                        <!-- <td class="text-center">
                            <?php //echo $data["Ready2Wear"] == 1 ? "<b class='text-success'>Ready-to-wear</b>".($data["Accessories"] == 1 ? "," : "" ) : "" ?>
                            <?php //echo $data["Accessories"] == 1 ? "<b class='text-warning'>Accessories</b>" : "" ?>
                        </td> -->
                        <td class="text-center sortable-hide-item">
                            <?php echo $data["Active"] == 1 ? "ใช้งาน" : "ไม่ใช้งาน" ?>
                        </td>
                        <td class="text-center sortable-hide-item">
                            <a href="productCategoryDetail.php?ref=<?php echo $data["CategoryCode"] ?>">
                                <i class="fa fa-cog"></i>
                            </a>

                            <form method="post" style="display:inline">
                                <button type="submit" class="btn-link" 
                                    onclick="return Confirm(this,'ต้องการลบรายการนี้หรือไม่ ?');"
                                    value="<?php echo $data["CategoryCode"] ?>" name="btnDeleteRow">
                                    <i class="fa fa-trash"></i>
                                </button>
                            </form>

                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>

        </div>
    </div>
</div>



<?php include  "../footer.php"; ?>