...
try{
    ...
    if($tempFile != false){
        ...
        $countAllRow = count($data) - 1;
        
        if($A1 != "ImportTicket-xlsx-zsx22ii8k1q&Tx"){
            throw new Exception("[E01] กรุณาอัพโหลดด้วยไฟล์เทมเพลต ที่ท่านดาวน์โหลดจาก ขั้นตอนที่ 1 เท่านั้น");
        }
        else if($countAllRow > 0 && count($data[0]) < 16){
            throw new Exception("[E02] กรุณาอัพโหลดด้วยไฟล์เทมเพลต ที่ท่านดาวน์โหลดจาก ขั้นตอนที่ 1 เท่านั้น");
        }
        else if($countAllRow == 0){
            throw new Exception("[E03] ไม่พบข้อมูล กรุณาเตรียมข้อมูลก่อน");
        }
